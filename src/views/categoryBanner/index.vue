<template>
  <div class="categoryBanner">
    ...
    <TablePager :data="categoryListData" :columns="column" :loading="loading" :pagination="{
            ...
            total: totalItem}" @change="onChange ">
      <template slot="cateImage" slot-scope="{row}">
        <img :src="row.cateImage" alt="" />
      </template>
      <template slot="recomend" slot-scope="{row}">
        <span>{{row.recomend | parseTime(formatTime)}}</span>
      </template>
      <template slot="operation" slot-scope="{row,index}">
        <span @click="clearRecomend(row,index)">取消推荐</span>
      </template>
    </TablePager>
    <!-- 新增推荐类目 -->
...
// import { getCategoryList, categoryChoose } from '@/api/categoryRecommend';
...
export default {
  data() {
    return {
      ...
      column: [
        ...
        { title: '图片', slot: 'cateImage' },
        { title: '推荐时间', slot: 'recomend' },
  ...
  methods: {
    ...
    async categoryData() {
      this.loading = true;
      let res = await getCategoryList(this.requestData);
      this.loading = false;
      let { categoryList, totalItem } = res.data;
